...
Visibility applies to top-level declarations, fields, and member functions. It does not apply to locals, parameters, statements, expressions, or plain imports. Fields and member functions inherit the enclosing type visibility unless explicitly narrowed. A field or member function cannot be more visible than its type. `export` is never inherited; write it explicitly on binary-visible member functions.
...
- C-facing aggregates use layout attributes: `[StructLayout(C)]` / `[StructLayout(Explicit)]`, `[Pack(N)]`, `[Align(N)]`, and `[FieldOffset(N)]`. Default Stark layout is not a stable ABI. `[StructLayout(C)]` plus `ffi(c)` is sufficient for by-value C aggregate parameters and returns; the compiler lowers the call through the target C ABI carrier, so use a C shim only for real signature adaptation. Parameter and result carriers are independent target facts: on AArch64 AAPCS64, a four-byte integer-like aggregate is passed through `i64` and returned through `i32`. A safe borrow of a misaligned packed field is rejected; a raw pointer to one is allowed and keeps the misalignment.
...
`Vendor.*` is a separate bundled vendor-library root for bindings to established native libraries. Use imports such as `import Vendor.Raylib`, `import Vendor.SQLite`, `import Vendor.GLFW`, `import Vendor.SDL3`, `import Vendor.Miniaudio`, `import Vendor.STB.Image`, or `import Vendor.Cgltf` for bundled native bindings. Keep vendor bindings ABI-shaped and low-overhead; use `[LinkName("NativeSymbol")]` for direct native symbol aliases and reserve C shim sources for real ABI adaptation such as callback bridges, lifetime conversion, C union flattening, C `bool` normalization, or single-header implementation boundaries. In an installed SDK, official vendor packages resolve through the exact `sdk.json` index and carry relative, checksummed native payload and link facts; applications do not add dependencies, invoke `pkg-config`, set `STARK_PATH`, or pass native search flags. `pkg-config`, optional native sources, and user-configured fallback paths are custom/source-package authoring inputs that must be resolved before an official package is released.
...
library as dependencies. In an installed SDK, `System.*` and `Vendor.*` imports
resolve through the exact module ownership index in `sdk.json`; a development
SDK may declare repository source/package roots explicitly. Use `[dependencies]`
...
Manifest discovery searches upward. The nearest `Stark.toml` runs in project mode; the nearest `Stark.solution.toml` runs in solution mode. Project commands route current host outputs through `build/<profile>/<target-triple>/stage0/`: executables/libraries under `bin/<project>/`, saved native intermediates under `obj/<project>/`, library package images under `pkg/<project>/`, and generated test runners/test executables under `tests/<project>/`. Installed `System.*` and official `Vendor.*` modules resolve only through the active SDK's exact `sdk.json` index; project ancestors and nearby `stdlib`/`vendor` directories are not searched. Stage and development source/package roots must be declared by their SDK manifests. Project builds ignore `STARK_PATH`, which remains a low-level direct compiler search-path escape hatch. If a reserved import cannot be resolved, diagnostics report the selected SDK/stage context plus active profile, target, and stage. `--target <triple>` sets both codegen target and output path; `--stage stage0` selects the current C# host stage, while Stage1/Stage2 execution remains future self-hosting work. `stark clean` defaults to the active stage scope and also supports explicit `profile`, `target`, `diagnostics`, and `artifacts` scopes. Project builds are incremental: each project's outputs carry an input-hash stamp (`.stark-build-stamp`) over its own sources, manifest, SDK manifest identity and selected package/source inputs, every dependency's stamp, the selected target/profile/`--filter`s, and the compiler binary itself, so an unchanged project is skipped and a changed one has its stale outputs (executable/library, generated test runner, intermediates, package image) cleared before recompiling. Only the projects whose inputs changed rebuild, so `stark test` pass/fail counts stay accurate without a manual `rm -rf build`; force a full rebuild with `stark clean` or by deleting the `build` directory.

Custom/source native-backed packages keep native metadata in the package
manifest. Official `Vendor.*` release packages already carry resolved,
SDK-relative payload and link facts and applications do not repeat this table:
