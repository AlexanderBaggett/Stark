...
public sealed class SystemThreadingStandardLibraryTests : StandardLibraryTestSuite
{
    ...
    public void StdLibSourceLinuxThreadingUsesRawCloneLifecycleAndSyscallBackedScheduler()
    {
        ...
        Assert.Contains("; direct-only asm definition omitted: LinuxMapThreadRegion", llvm, StringComparison.Ordinal);
        Assert.Contains("; direct-only asm definition omitted: LinuxReleaseThreadReference", llvm, StringComparison.Ordinal);
        Assert.Contains("; direct-only asm definition omitted: LinuxCloneThread", llvm, StringComparison.Ordinal);
        ...
        Assert.Contains("lock xaddl %eax, 32(%rdi)", llvm, StringComparison.Ordinal);
        Assert.Contains("movq $$56, %rax", llvm, StringComparison.Ordinal);
