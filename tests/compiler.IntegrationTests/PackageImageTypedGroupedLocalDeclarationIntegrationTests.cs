...
public sealed class PackageImageTypedGroupedLocalDeclarationIntegrationTests
{
    [Fact]
    public async Task ManifestBackedTypedGroupedLocalDeclarationsCompileAndRunWithoutSyntheticSource()
    {
        ...
        var tempDirectory = Directory.CreateTempSubdirectory("stark-package-image-typed-grouped-local-runtime-");
        ...
        try
        {
            ...
            var template = Assert.Single(facadeModule.GenericTemplates!.Functions, static template => template.QualifiedResolvedName == "Facade.SumTo");
