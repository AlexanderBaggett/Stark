...
public sealed class PackageImageTypedTerminalIfIntegrationTests
{
    [Fact]
    public async Task ManifestBackedTypedTerminalIfBodiesCompileAndRunWithoutSyntheticSource()
    {
        ...
        var tempDirectory = Directory.CreateTempSubdirectory("stark-package-image-typed-terminal-if-runtime-");
        ...
        try
        {
            ...
            var template = Assert.Single(facadeModule.GenericTemplates!.Functions, static template => template.QualifiedResolvedName == "Facade.ChooseTerminal");
