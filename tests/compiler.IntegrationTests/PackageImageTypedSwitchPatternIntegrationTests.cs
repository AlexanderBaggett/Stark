...
public sealed class PackageImageTypedSwitchPatternIntegrationTests
{
    ...
    [Fact]
    public async Task ManifestBackedTypedRangeSwitchPatternsCompileAndRunWithoutSyntheticSource()
    {
        if (!NativeToolchain.TryDetectDefaultTargetInfo(out _))
        {
            return;
        ...
        try
        {
            ...
            Assert.Contains(
                ...
                statement => statement.SwitchCases?.Any(static switchCase => string.Equals(switchCase.Kind, "range", StringComparison.Ordinal)) == true);
            ...
            Assert.Equal(string.Empty, processOutput);
            Assert.Equal(string.Empty, processError);
            Assert.Equal(27, process.ExitCode);
        }
        finally
        {
            try
