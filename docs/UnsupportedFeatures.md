...
- Generic and package-image gaps: imported generic specialization now loads manifest-backed typed-interface and compiler-fact sections directly, and supported generic bodies can be seeded from typed template summaries during structured package-image loading instead of depending on bridge source text; that supported subset now stays declaration-only during structured package-image loading instead of being re-rendered into fake source bodies, and package publication now omits duplicated raw body text for those supported typed templates; the temporary synthetic-source bridge remains only as a compatibility fallback for templates that still lack sufficient typed summaries, but typed-interface sections now also carry published overload identity, generic-body availability, and authored hot/cold/inline modifier identity directly so supported generic declaration loading, bridge body recovery, and imported planning no longer require source-surface function/type entries or compiler-fact sections for that surface, and generic template publication now also preserves weighted body-cost summaries plus declared/effective function-kind, semantic call-graph, parameter/memory-effect summaries, and structural optimization summaries for wrappers/calls/accesses/operators/unary operators/terminal branch-selection wrappers/branches/loops/object creation so imported monomorphization planning, effective-kind refinement, direct/member-call, field/index-access, simple binary/comparison, terminal `if`/`switch` selector, explicit-conversion, and simple address-of or raw-pointer-dereference wrapper-inline promotion, caller-clone eligibility, and LLVM attribute emission do not rely only on top-level statement count or top-level function-semantics sections, including the current top-level imported-helper and root-owned specialization-symbol caller-clone surface; published typed-template bodies now cover direct switch sections over literal/default/match-all/guarded labels, simple switch-pattern helpers over already-published enum and aggregate pattern facts including literal field tests and nested enum/aggregate field subpatterns, `if`/`while`/`for` control flow plus `break`/`continue`, including terminal non-void `if`/`else` chains whose branches all return, ternary call statements, grouped local variable declarators including uninitialized declarators, object-initializer variable initializers, and other supported initializers, grouped local constant declarators with supported initializers, grouped `for`-initializer local declarators with uninitialized declarators, object-initializer variable initializers, or other supported initializers, discarded supported expression statements, binary operator expressions including exponentiation, equality and ordered comparison chains with once-evaluated semantics, assignment expressions over the same supported local and named-root field/index target subset as published assignment statements plus raw-pointer dereference-root targets including projected field/index chains rooted at `*expr`, unary address-of expressions over that same supported addressable target subset, direct raw-pointer dereference unary expressions, the full currently supported text postfix bracket family (`text[]`, `text[index]`, and `text[start, length]`), fixed-array local initializers plus supported nested fixed-array and object-initializer member values inside published object-creation expressions, and local plus named-root field/index assignments, including compound assignments, under the same non-const local assignment rules as source lowering; nested generic type layouts now propagate through use-site planning, typed-interface sections now carry structured import dependency surface so bridge reconstruction no longer depends on explicit source-surface imports for that path, and general published template bodies are not yet full typed template IR.
