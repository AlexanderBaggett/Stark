...
- Operator and type gaps: dereference requires raw pointers; exponentiation requires a floating-point common type; integer-only operator chains reject non-integer common types; ordered comparison lowering is still limited to integer, float, bool, raw-pointer, and same-kind `ascii`/`unicode` families, while equality and inequality now also support same-kind `ascii`/`unicode`, same-kind slices, and scalarizable aggregates over those leaf families.
...
- Generic and package-image gaps: imported generic specialization still relies on the temporary synthetic-source bridge outside the published typed-template subset; published typed-template bodies now cover direct switch sections over literal/default/match-all/guarded labels, ternary call statements, and local plus named-root field/index assignments, including compound assignments, and typed-interface sections now carry structured import dependency surface so bridge reconstruction no longer depends on explicit source-surface imports for that path, but general published template bodies are not yet full typed template IR.
...
- Indexing gaps: slice and raw-pointer indexing require integer indices; indexing is only supported for fixed arrays, raw pointers, slices, `ascii`, and `unicode`; text slicing currently requires exactly two integer indices.
