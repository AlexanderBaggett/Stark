...
| .NET collections | `Dictionary`, `HashSet`, `List`, `Queue`, `Stack`, `IReadOnlyList`, `IReadOnlyDictionary`, `IReadOnlySet` throughout `src/Compiler` | Requires Stark collection parity, especially string-key dictionaries and sets |
...
| `System.Numerics.BigInteger` | `src/Compiler/TypeChecking.cs`, `src/Compiler/CompileTimeExpressionEvaluator.cs`, `src/Compiler/IntegerRangeStorageFacts.cs`, `src/Compiler/EnumLayoutBuilder.cs`, SSA value facts | Requires arbitrary precision or a proven Stark integer facade sufficient for all range/literal work |
...
| `System.Text.Json` | Package image load/write and CLI inspection | Requires JSON support or a package format decision |
...
| `Stark.toml` | Projects | `ProjectCliDriver` | Parsed by host `SimpleToml`; Stark stdlib lacks TOML support |
| `Stark.solution.toml` | Solutions | `ProjectCliDriver` | Build/run/test target discovery |
| `Stark.user.toml` / user config TOML | Local config | `ProjectCliDriver` | Tool/native path overrides |
| `.starkpkg.json` | Package image builder | Package image loader, CLI inspector | Compiler-owned JSON package image, not a hand-authored manifest |
