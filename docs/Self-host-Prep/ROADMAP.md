...
- [~] Add reusable `System.Toml` parser/emitter support and use it for `Stark.toml`, `Stark.solution.toml`, and `Stark.user.toml` manifest decoding. The reader landed in `stdlib/src/System/Toml.stark` (flat-node document with line/column spans, span-carrying diagnostics, tables/dotted keys/inline tables/arrays/strings with escapes/decimal integers/booleans/comments — the staged manifest subset; floats/datetimes/multiline strings/arrays-of-tables report UnsupportedValue) with 17 stark-test facts in `tests-stark/stdlib.Toml`, including decoding the repo's real project-manifest shape. The deterministic emitter, file helpers, typed manifest decoding for the self-hosted driver, and the remaining TOML 1.x surface stay tracked in [21-system-toml.md](21-system-toml.md#7-work-items); see also [02-stdlib-gaps.md](02-stdlib-gaps.md), [03-tooling-gaps.md](03-tooling-gaps.md), and [07-open-questions.md](07-open-questions.md).
...
- [ ] Implement test collections: variadic `[Collection("a", "b")]` on modules, types, and facts (union semantics), runtime filtering in the generated runner, and repeatable/comma-list `stark test --collection` selection — targeted test slices for the stark-side suites. Track detailed items in [language-ergonomics-checklist.md](language-ergonomics-checklist.md).
