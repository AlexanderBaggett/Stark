...
- [~] Port parser, diagnostic, type checking, ownership, MIR, SSA, LLVM, and package tests. `tests-stark/compiler.Tests` now runs 383 ported facts green against the host compiler (DiagnosticRegression 113, TypeTypingDiagnostics 86, SemanticValidation 82, OwnershipRoadmap/OwnershipValidation 60, SwitchExhaustiveness/TryPropagation/IfWhilePattern 40, parser smoke 2; 9 PORT-NOTEs need artifact-model or target-pinning protocol support). LLVM emission, SSA, package-image, and runner-generator categories follow. See [05-port-checklist.md](05-port-checklist.md).
...
- [~] Add compiler-grade text building, formatting, escaping, and diagnostic rendering helpers. Stark literal escaping/decoding helpers have landed in `System.Text`, along with `SplitAscii`/`SplitUnicode` (splitting a view into a `List<OwnedAscii>`/`List<OwnedUnicode>`), the `RepeatAscii`/`RepeatUnicode` family, and the owned-text builder slice; diagnostic rendering and golden/snapshot support remain. See [02-stdlib-gaps.md](02-stdlib-gaps.md).
