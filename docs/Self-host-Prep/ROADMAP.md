...
- [~] Add compiler-grade text building, formatting, escaping, and diagnostic rendering helpers. Stark literal escaping/decoding helpers have landed in `System.Text`, along with `SplitAscii` splitting a view into a `List<OwnedAscii>` and the owned-text builder slice; diagnostic rendering and golden/snapshot support remain. See [02-stdlib-gaps.md](02-stdlib-gaps.md).
...
- [ ] Implement test collections: variadic `[Collection("a", "b")]` on modules, types, and facts (union semantics), runtime filtering in the generated runner, and repeatable/comma-list `stark test --collection` selection — targeted test slices for the stark-side suites.
