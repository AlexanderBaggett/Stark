...
- [~] Add process execution with optional stdin, stdout, stderr, exit-code, timeout, argv, environment, and working-directory capture. Linux-backed `System.Process` APIs and `System.Testing` process output/run-match/timeout helpers have landed; cross-platform backend parity remains. See [04-test-infrastructure-audit.md](04-test-infrastructure-audit.md) and [02-stdlib-gaps.md](02-stdlib-gaps.md).
- [~] Add a host-compiler target mode so Stark tests can run against the current C# compiler. Initial `compiler --host-test-inspect` / `--host-test-server` bridge exists; Stark-side wrappers and staged runner integration remain. See [04-test-infrastructure-audit.md](04-test-infrastructure-audit.md).
...
- [~] Add process spawning, environment, argv, working-directory, timeout, and stdin/output-capture APIs. Linux-backed `System.Process` APIs and focused tests have landed; cross-platform backend parity remains. See [02-stdlib-gaps.md](02-stdlib-gaps.md).
