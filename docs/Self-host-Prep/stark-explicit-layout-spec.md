# Stark Language Feature Spec: Explicit Memory Layout (Bitfields & Field Offsets)
...
> Note: The attribute decorator form `[...]` used throughout is illustrative. Adjust to Stark's
> actual attribute/modifier syntax once finalized; the semantics below are the substantive part.

---

## 1. Motivation

Stark's current FFI is limited for C interop. Two related layout capabilities are missing:

1. **Field offsets (unions)** — overlapping fields at explicit byte offsets, e.g. a `u64` whose
   bytes are also addressable as four `u16`s. Needed for FFI, wire formats, and reinterpreting memory.
2. **Bitfields** — named sub-byte regions packed into a backing integer, e.g. a 3-bit mode and a
...
## 2. Field Offsets (Explicit-Layout Unions)
...
### 7.1 Reinterpreting a 64-bit word as four 16-bit lanes
...
### 7.2 A hardware status register
