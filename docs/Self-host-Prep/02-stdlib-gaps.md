...
| S06 | Compiler-grade collections | `List<T>`, `Stack<T>`, `Queue<T>`, `LinkedList<T>`, `Dictionary<K,V>`, `Lookup<T>` documented; `Dictionary<K,V>` now has scalar fast paths plus explicit static `Hash`/`Equals` support for non-primitive keys | Blessed OQ-08/doc `19` model: `Dictionary<K,V>` and `HashSet<T>` use public `Hash` + `Eq` contracts; ordered maps/sets and sorting use `Ord`; text key types provide reusable contracts; package images/diagnostics use explicit deterministic order; borrowed lookup avoids allocation | blocker |
...
| `System.Collections` | `List<T>`, `Stack<T>`, `Queue<T>`, `RingQueue<T>`, `LinkedList<T>`, `Dictionary<K,V>`, `Lookup<T>`; dictionary key support has bool/integer fast paths and explicit static `Hash`/`Equals` for non-primitive keys | `Dictionary<Ascii,T>`, `Dictionary<Unicode,T>`, `HashSet<T>`, deterministic ordered maps/sets, sorting, iteration, value-update APIs, borrowed lookup APIs, and canonical `Hash` + `Eq` wording | S06, S07, S08 |
...
| `System.Text` | Encodings, owned text, conversions, parsing/formatting many integer widths, text errors/results | Compiler-grade builders, exact ordinal text hashing/equality/ordering contracts, escaping, joining, casing, splitting, efficient append, stable formatting | S02, S03, S06 |
