...
| OQ-05 | Compiler invariant failure policy L07 | A. Provide `System.Diagnostics.Trap/Unreachable/Assert` style APIs. B. Model every invariant as recoverable `Result`. C. Allow compiler-internal unsafe trap helpers only. | A gives clear intent; B makes impossible-state code noisy and can obscure bugs; C keeps scope small but needs documentation so accepted-program bugs do not become UB. |
