...
| T02 | Bootstrap staging with C# Stage0 | Stage0 builds Stage1; Stage1 builds Stage2; qualified self-hosted compiler becomes the release compiler while Stage0 remains available | Policy specified: retain the existing C# compiler in `/src` as Stage0, keep the Stark compiler in `/selfhost`, and do not require source moves or host-code removal; no separate blessed snapshot compiler artifact | blocker |
...
| T14 | Release packaging/doctor/clean-machine verification | Re-establish or update release builds to distribute the qualified self-hosted compiler, bundled toolchain, and stdlib while retaining an explicit Stage0 build | Roadmap exists but unchecked | blocker before self-hosted release adoption |
