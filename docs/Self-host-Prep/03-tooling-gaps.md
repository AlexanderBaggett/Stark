...
| Build artifacts | Project driver routes Stage0 project outputs through the formal `.stark/build/<profile>/<target-triple>/stage0/` layout for `bin`, `obj`, `pkg`, `tests`, stage-local `stdlib` lookup, and clean scopes; diagnostics/artifacts/stdlib generation and real Stage1/Stage2 execution remain open | `ProjectCliDriver`, integration tests, `docs/Self-host-Prep/25-build-artifact-layout.md` |
...
| T05 | Stable `.stark/build/` artifact layout | Bootstrap artifacts, package images, host/stage compiler outputs, targeted diagnostics, native objects | Partial: stage0 project builds now resolve/pass a target triple and route final executable/library outputs to `bin/<project>/`, generated test runners and test executables to `tests/<project>/`, saved native intermediates to `obj/<project>/`, and project library package images to `pkg/<project>/` under `.stark/build/<profile>/<target-triple>/stage0/`; `stark clean` covers profile/target/stage/diagnostics/artifacts scopes; project builds search the active stage-local `stdlib/` path and nearest repo `stdlib/dist` / `stdlib/src` development paths. Stdlib artifact generation/routing, diagnostics/artifact export routing, and Stage1/Stage2 routing remain open | blocker |
