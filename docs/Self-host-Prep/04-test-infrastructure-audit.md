...
| TEST-05 | Process execution capture | Linux- and macOS-backed `System.Process.RunCapture`, `RunCaptureWithInput`, and timeout variants capture stdout/stderr/exit code, can feed stdin, and expose argv/env/cwd helpers through the platform dispatch layer; `System.Testing` has process output assertions plus run-match and timeout helpers; the Windows process backend remains stubbed | Basic CLI/native/project/runtime assertions can now use the harness on Linux and macOS; the Windows process backend still gates Windows-target ports |
| TEST-06 | Host-compiler target mode for ported tests | Host bridge landed (`compiler --host-test-inspect` / `--host-test-server`), and the Stark-side `System.Testing.HostCompiler` wrapper now spawns the server via `System.Process`, writes protocol requests with `System.Json`, parses structured responses (success, error counts, diagnostics, artifact texts), and backs the first green `stark test` projects under `tests-stark/` using `STARK_HOST_COMPILER` | Request batching, `searchDirectories`, `System.Testing.Diagnostic` adaptation, artifact-file output, and per-port convenience helpers remain |
