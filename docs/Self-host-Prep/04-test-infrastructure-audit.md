...
| TEST-05 | Process execution capture | Linux-, macOS-, and Windows-backed `System.Process.RunCapture`, `RunCaptureWithInput`, and timeout variants capture stdout/stderr/exit code, can feed stdin, and expose argv/env/cwd helpers through the platform dispatch layer; `System.Testing` has process output assertions plus run-match and timeout helpers | Basic CLI/native/project/runtime assertions can now use the harness on Linux, macOS, and Windows; executable-permission helpers remain separate platform work |
