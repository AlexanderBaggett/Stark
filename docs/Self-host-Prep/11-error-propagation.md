...
propagation and drop correctness through funnels), `System.Compiler` CTFE facts
for inspecting `from` funnel metadata, and user-facing docs.
...
| EP20 | Source-module imports: lower `try` inside imported *source* module bodies (type checking skips imported non-generic bodies, so no typing record exists for those sites) | **done** | `DeriveImportedSourceTryPropagation` in MIR lowering re-derives the roles/funnel record from the lowered operand type and the enclosing return type, mirroring `EvaluateTryExpression`/CTFE; the fallback named-type collection now preserves `[Ok]`/`[Err]` roles and `from` `AbsorbsErrorType` markers for imported source enums (`BuildFallbackEnumVariant`); regression: `TryPropagationImportedModuleTests` (same-error, funnel, unit, binding/statement/return positions, generic operand); `System.Testing.HostCompiler` consumes `try` idiomatically as the first stdlib user |
