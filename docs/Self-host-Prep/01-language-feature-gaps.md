...
| L10 | Async/build-driver concurrency replacement | `src/Compiler/ProjectCliDriver.cs` and `src/Compiler/CompilerCli.cs` expose `async Task<int>` and use async stream/process APIs | workaround-exists | **design drafted (doc `12`)** | Stark has no `async`/`await` surface and will not add one for this; doc `12` frames the replacement as thread ownership (scoped borrowing threads + owning move-capture threads + lock-owns-data containers, all over existing ownership rules). A synchronous driver port remains viable first; OQ-TH1..6 need locks before threaded orchestration. |
