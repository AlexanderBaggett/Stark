...
Status: accepted architecture, implementation paused while Stage0 release work
has priority. Remaining implementation work is tracked only in
...
- Build artifacts continue to use the
  ...
  [the build artifact layout](BuildArtifactLayout.md).
...
**Decision: `System.*` and official `Vendor.*` discovery is rooted in one
explicit SDK manifest.** The canonical contract is documented in
[SDK Layout and Resolution](SdkLayoutAndResolution.md).

SDK resolution is strictly `--sdk-root`, then `STARK_SDK_ROOT`, then one of the
bounded executable-relative layouts. Every selected SDK has a canonical root
and `sdk.json`; the compiler must not search project ancestors, ambient global
package locations, or the network. The manifest's module index identifies the
exact owning package and its package image, wrapper archive, native payload,
runtime files, and ordered link arguments. Stage-local and repository-source
fallbacks are expressed by `stage` or `development` SDK manifests rather than
additional implicit search tiers.

If discovery fails, diagnostics must report the selected root or every bounded
candidate plus the active target, profile, and compiler stage. Ordinary
third-party package dependencies remain explicit project/package references
unless a future package-manager decision adds a separate layer.
...
[System.Threading](../StandardLibrary/System.Threading.md):
...
[System.Collections](../StandardLibrary/System.Collections.md) and
[System.Text.Interning](../StandardLibrary/System.Text.Interning.md).
...
`System.Toml`.** See [System.Toml](../StandardLibrary/System.Toml.md).
...
**Invariant failure convention.** Stark deliberately has
...
See [IR Memory and Fact Model](IrMemoryAndFactModel.md).
