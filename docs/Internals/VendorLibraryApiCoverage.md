# Vendor Library API Coverage

This durable coverage snapshot records the audited upstream native-library
surfaces, Stark's current bundled `Vendor.*` coverage, intentional equivalents,
and unbound API groups. Prioritized implementation work is tracked only in
[`docs/Self-host-Prep/TASKS.md`](../Self-host-Prep/TASKS.md); the pinned headers
and generated inventories remain the authoritative symbol lists.
