...
Status: accepted backend architecture. Remaining implementation work is tracked
...
4. Shelling out to `clang`/`lld` remains for final platform linking, native C
   source compilation, SDK/private-backend discovery, host-link discovery, and
...
| Library form | Prefer a compiler-private dynamic libLLVM runtime in release archives; static linking is a later packaging decision. Never treat it as a public LLVM development kit. |
...
| Link step | Resolve the documented host link layer separately from the packaged compiler backend. |
...
| Native dynamic library discovery/loading policy | The self-hosted compiler must locate the packaged compiler-private or explicitly overridden libLLVM for the selected target/platform. | Release/backend packaging roadmap |
| Native link metadata for libLLVM | The compiler package/build must know which LLVM runtime libraries it owns and which platform link inputs belong to the documented host layer. | Release/backend packaging roadmap |
...
- Add package/release tests that compile a program using only the packaged
  compiler-private libLLVM runtime and the documented Odin-style host link layer.
