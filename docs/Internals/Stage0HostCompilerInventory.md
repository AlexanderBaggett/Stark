# Stage0 Host Compiler Inventory

This is the retained May 27, 2026 audit snapshot of the C# Stage0 compiler that
seeded self-hosting work. File counts and the exhaustive manifest are historical;
the repository is authoritative for current paths. The subsystem, tool, format,
and alias/noalias inventories remain useful for Stage0 maintenance and eventual
self-hosting resumption.
...
| External parser runtime | `Antlr4.Runtime.Standard` package and checksum-pinned generated files are aligned at ANTLR `4.13.1` |
...
| `System.Numerics.BigInteger` | `src/Compiler/TypeChecking.cs`, `src/Compiler/CompileTimeExpressionEvaluator.cs`, `src/Compiler/IntegerRangeStorageFacts.cs`, `src/Compiler/EnumLayoutBuilder.cs`, SSA value facts | Host implementation convenience only; the self-host architecture replaces it with bounded compiler-internal `i1024`/`u1024` integer-fact helpers and diagnostics for overflow/oversized values |
...
| `System.Text.Json` | Current host package image load/write and CLI inspection | The [package-image design](PackageImage.md) replaces normal package loading with binary package images; deterministic JSON/text remains for inspection/export and tests |
...
| ANTLR generator | Generated `src/Parsing/*` from `Stark.g4`; exact generator/runtime version `4.13.1` | Parser source generation | Stage0 input is pinned; self-host still replaces the ANTLR dependency with the Stark-native parser |
...
| `ld.lld` / `lld-link` | `NativeToolchain.SupportsExecutableThinLto`, linker selection | ThinLTO and linking | Only where the allowlisted Stage0 private backend or qualified host-link policy requires it |
...
| platform SDK tools/libs | native link on Linux/macOS/Windows | CRT/libc/SDK resolution | Yes, even with the packaged compiler-private backend |
