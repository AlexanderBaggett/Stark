...
- [x] Add whole-program or LTO-style optimization for Stark executables so stdlib calls can inline into user code. This is probably the highest-leverage fix.
  - [x] add the first executable ThinLTO wiring
  - [x] re-enable executable ThinLTO after fixing the packaged `System.Collections` grow/move/drop runtime spin
...
- [x] Add an SSA value-fact model artifact
  ...
  - [x] represent integer ranges, known bits, boolean constants, known-null/non-null pointers, known pointer alignment, and known slice/text lengths
    - [x] represent integer ranges from source types, constants, simple arithmetic, conversions, and phis
    - [x] represent boolean constants proven from simple range comparisons
    - [x] represent known bits for integer singleton constants
    - [x] represent known zero bits for nonnegative bounded integer ranges
    - [x] represent known-null and known-non-null facts for obvious null/address values
    - [x] represent exact fixed-array slice lengths and direct text-slice length ranges
    - [x] represent exact text literal lengths for `ascii` and `unicode` constants
    - [x] infer known pointer alignment beyond the field reserved in the fact model
    - [x] infer slice/text length ranges beyond fixed arrays and direct text-slice lengths
  - [x] include block-entry and block-exit fact sets so facts learned from branches can be scoped correctly
    - [x] include branch-derived block-entry integer range facts for simple comparison edges
    - [x] include block-exit fact sets carrying branch-scoped entry facts and useful facts for values defined in each block
  - [x] make the fact lattice explicit: unknown, known fact, conflicting/overdefined fact
  - [x] add diagnostic/log hooks so optimization traces can explain which facts were learned or rejected
    - [x] emit a verbose `ssa.value-facts.summary` optimization log with learned fact counts by category plus block-entry/block-exit fact counts
  - [x] add unit tests for fact joining at phis, branch targets, loops, and unreachable blocks
    - [x] add tests for type-derived ranges, proven comparisons, and phi range joins
    - [x] add tests for dependent range propagation after phi joins
    - [x] add tests for fixed-array slice lengths and direct text-slice length ranges
    - [x] add tests for branch target entry facts
    - [x] add tests for loop phi joins and unreachable phi inputs
...
- [x] Add an SSA value-range and proof-propagation pass
  ...
  - [x] prove tighter result ranges for non-wrapping arithmetic where Stark's ordinary arithmetic makes overflow undefined
    - [x] clamp ordinary arithmetic ranges to the continuing destination type range and avoid invalid min-greater-than-max facts when every mathematical result would overflow
  - [x] preserve separate handling for wrapping and saturating arithmetic so proof facts do not erase required semantics
    - [x] keep wrapping arithmetic facts conservative when operand ranges may wrap
  - [x] propagate known slice length, text length, and fixed-array length facts through slice/text construction and indexing operations
    - [x] propagate fixed-array lengths through slice construction
    - [x] propagate direct text-slice length operand ranges through text construction
    - [x] propagate exact text literal lengths through `System.Text.AsciiLength` and `System.Text.UnicodeLength`
  - [x] propagate branch facts such as `x < n` on the true edge and `x >= n` on the false edge
    - [x] propagate simple integer comparison range facts onto branch target entry facts
  - [x] propagate pointer facts from null checks, equality checks, borrow-derived non-null values, and address-of operations
    - [x] propagate raw-pointer nullability facts from `== null` and `!= null` branch edges
    - [x] propagate raw-pointer non-null facts from equality edges when either side is already known non-null
  - [x] add tests showing narrower ranges after branches, phis, casts, arithmetic, and loops
    - [x] add tests for bitwise and shift range propagation
    - [x] add tests for branch-target range facts
    - [x] add branch-pruning regression coverage for bitwise-derived ranges
    ...
    - [x] add branch-pruning regression coverage for explicit wrapping and saturating arithmetic facts

- [x] Add fact-driven branch and switch pruning
  - [x] add a rewrite pass after `value-facts` that consumes the SSA fact artifact
  - [x] fold comparisons proven always true or always false
  - [x] rewrite branches with proven conditions into gotos
  - [x] remove switch cases whose match value is outside the proven input range
  - [x] rewrite switches with one reachable case into direct gotos or simple branches
  - [x] remove blocks that become unreachable, then rerun SSA cleanup and constant propagation
  - [x] keep branch-weight metadata valid after deleting or merging edges
  - [x] add tests proving dead switch arms, impossible branches, and unreachable phis are removed

- [x] Feed value facts into LLVM emission
  - [x] teach LLVM emission to consume the SSA value-fact artifact instead of relying only on local type-based range queries
  - [x] emit stronger `range` metadata for loads and returns when propagated facts are narrower than source types
    - [x] emit narrower return `range(...)` attributes from propagated SSA return-value facts
    - [x] prefer propagated SSA value facts when emitting `!range` metadata for load and call results
  - [x] emit stronger `nuw`, `nsw`, and `exact` flags on arithmetic, division, and shifts when facts prove the contracts
  ...
  - [x] emit stronger `inbounds`/`nuw` GEP flags for fixed-array, slice, text, and aggregate element accesses when index facts prove object bounds
  ...
  - [x] emit `llvm.assume` for facts that are valuable to LLVM but not otherwise visible in IR
  - [x] add LLVM tests that compare before/after IR for range metadata, arithmetic flags, GEP flags, and assumptions
    - [x] add LLVM test for propagated phi range producing stronger no-wrap flags
    - [x] add LLVM test for propagated value facts producing narrower return range attributes
...
- [x] Add direct-call devirtualization for known function pointers
  - [x] add a standalone SSA rewrite pass after cleanup/constant propagation and before `inline-ssa`
  - [x] move the direct-call rewrite out of cleanup/constant propagation once standalone pass ordering became useful
  - [x] detect `SsaIndirectCallRValue` targets that are directly `SsaFunctionAddressValue`
  - [x] detect targets loaded through phis where every reachable incoming value is the same function address
  - [x] rewrite proven indirect calls into `SsaCallRValue` direct calls
  - [x] preserve indirect-call target and argument operands through SSA cleanup use tracking
  - [x] preserve address-taken function facts when a function pointer value still escapes or is stored
  - [x] remove address-taken function facts when devirtualization eliminates the only address use
  - [x] add tests for direct fnptr calls, lambda calls, phi-joined identical targets, and non-devirtualized mixed targets
    - [x] direct fnptr calls
    - [x] phi-joined identical targets
    - [x] non-devirtualized mixed targets
    - [x] lambda calls
  - [x] add a Stark/C/Rust microbenchmark for known function-pointer devirtualization

- [x] Add Stark-level direct-call inlining
  ...
  - [x] inline `law` and `finite law` helpers more aggressively than ordinary functions
  ...
  - [x] clone and inline call sites with constant arguments when the clone unlocks branch pruning, range narrowing, or aggregate scalarization
  - [x] refuse inlining for `ffi`, `cold`, explicitly `noinline`, recursive, or unsupported direct-codegen bodies in the first pass
  - [x] rerun SSA cleanup and constant propagation after inlining
  - [x] rerun value-facts and branch pruning after inlining once those passes exist
  - [x] add tests showing small direct-call helpers disappear from optimized SSA and LLVM
...
- [x] Add whole-program executable optimization for stdlib and package-image code
  - [x] re-enable executable ThinLTO for optimized source-built executable dependency graphs when an LLD linker is available
  - [x] fix the `System.Collections` grow/move/drop ThinLTO correctness issue so collection-using executables can participate
    - [x] ThinLTO bitcode emission now skips `-disable-llvm-passes`; focused `SourceStdLibCollectionsGrowMoveDropExecutableRuns` passes with root and `System.Collections` ThinLTO enabled on April 28, 2026
  - [x] fix the `System.Memory`/`System.Text`/root ThinLTO correctness issue exposed by `benchmarks/text/OwnedTextAllocation`
    - [x] remove the temporary `System.Memory` native-only dependency gate after validating `OwnedTextAllocation` with root, `System.Text`, and `System.Memory` all emitted as ThinLTO bitcode on April 28, 2026
  - [x] extend package-library emission/linking so package-image code can safely participate in executable LTO
    - [x] optimized `--emit-lib` now archives ThinLTO bitcode objects when the toolchain supports executable ThinLTO, and manifest-backed executable links no longer disable root/link ThinLTO just because a Stark package library is present
  ...
  - [x] add pipeline or LLVM regression coverage proving a root executable can optimize through a stdlib/package-image call boundary
    - [x] `InlineSsaOptimizesThroughSourceBuiltDependencyBoundary` proves a root module can inline and constant-fold through a source-built dependency boundary before LLVM emission
  - [x] re-run the ASCII-to-Unicode benchmark to measure the closed-world optimization impact

- [x] Add a dynamic whole-program optimization policy for ThinLTO and LLVM pass participation
  - [x] model each source-built dependency module with optimization-safety facts: can emit ThinLTO bitcode, can run normal LLVM passes, contains known fragile constructs, and exposes hot inline candidates
  - [x] replace broad module-name gates such as `System.Collections` with a policy decision that records the exact reason a module or executable is kept out of ThinLTO
    ...
    - [x] remove the temporary `System.Memory` name gate after fixing ThinLTO bitcode emission
  - [x] choose root-module, dependency-module, and link-time optimization modes independently so a root executable can still benefit when one dependency must remain native
  - [x] add diagnostics or toolchain metrics that explain which optimization mode was selected for each module and why
  - [x] add regression tests for mixed graphs where one module is ThinLTO-safe, one module is intentionally native-only, and the root still optimizes through eligible call boundaries
  ...
  - [x] add source support for `[Backend(Opaque)]` as a function-level backend optimization boundary attribute
    ...
    - [x] add diagnostics for unsupported placements, duplicate backend attributes, and conflicts with existing function modifiers such as `inline`, `noinline`, `ffi`, `cold`, and `hot`
      - [x] reject contradictory callable modifiers `inline`, `inlinehint`, `noinline`, `hot`, `cold`, and `ffi` when paired with `[Backend(Opaque)]`
  ...
  - [x] add source support for `[Backend(Opaque)]` as a `struct`, `record`, and `doctrine` backend optimization boundary attribute
    ...
    - [x] define the exact lowering semantics for type-level opacity: constructors, destructors, methods, generated drop/move helpers, and monomorphized type-owned helper functions must inherit the boundary unless a narrower rule is explicitly chosen
    - [x] define the exact lowering semantics for doctrine-level opacity: generated doctrine dispatch/helper functions and concrete doctrine constraint implementations must preserve the boundary where they can affect backend optimization
    ...
    - [x] add diagnostics for unsupported placements, duplicate backend attributes, and conflicts between module-level, type-level, and callable-level backend modes
      - [x] v1 exposes only `Opaque`, so nested module/type/callable opacity is monotonic; duplicate attributes and unsupported non-callable placements are rejected
    - [x] add regression tests proving opaque `struct`, `record`, and `doctrine` declarations block cross-boundary backend optimization only for the affected generated symbols
    - [x] migrate `System.Collections` from module-level opacity to the narrowest function/type/doctrine-level opacity that preserves correctness and benchmark performance
      - [x] make `[Backend(Opaque)]` apply only to `Dictionary<K, V>` first if dictionary growth/move/drop is the only remaining collection boundary that needs containment
      - [x] remove module-level `[Backend(Opaque)]` from `System.Collections` for the first dictionary-only trial while keeping `List<T>`, `Stack<T>`, `Queue<T>`, and `LinkedList<T>` eligible for normal backend optimization
      - [x] verify generic monomorphization is not bypassing `[Backend(Opaque)]` boundaries more than intended; opaque generic functions, methods, and type-owned helpers must preserve the boundary after concrete instantiation
    - [x] rerun `benchmarks/collections/DictionaryLookup`, `ListGrowth`, `ListIteration`, `QueueGrowth`, `StackGrowth`, and `LinkedListPush` before and after the migration and record same-run C ratios
      - [x] post dictionary-only opacity trial, focused 20-run sample on April 28, 2026: `DictionaryLookup` ratio 1.071, `ListGrowth` 0.998, `ListIteration` 0.969, `QueueGrowth` 0.998, `StackGrowth` 1.052, and `LinkedListPush` 1.028 versus same-run C baselines
      - [x] focused 20-run validation on April 28, 2026: `DictionaryLookup` ratio 1.049, `ListGrowth` 1.051, `ListIteration` 1.077, `QueueGrowth` 1.024, `StackGrowth` 1.008, and `LinkedListPush` 1.061 versus same-run C baselines
  ...
  - [x] document the policy so performance-sensitive users can understand when code is optimized as a closed world and when Stark deliberately keeps a boundary opaque
...
- [ ] Add slab/page refill for allocator buckets
  ...
  - [ ] measure memory retention and platform-specific behavior on Linux, Windows, and other supported targets
    - [x] document current Linux/Windows retention policy: bucket slabs are retained on thread-local bucket lists for process-lifetime reuse, while large and over-aligned allocations return to the OS-backed free path immediately
    - [x] add explicit `peak_rss_kib` benchmark-harness output for Linux Bash and Windows PowerShell runners so allocator-retention probes can report memory as well as time
      - [x] Bash smoke on April 28, 2026: `STARK_BENCH_CAPTURE_RSS=1 STARK_BENCH_RUNS=1 STARK_BENCH_FILTER='benchmarks/micro/Calls.stark' STARK_BENCH_LANGUAGES='c' scripts/run-benchmarks.sh` produced the new `peak_rss_kib` column and preserved `c_avg_ratio`
      - [ ] validate the PowerShell runner on a Windows host; this Linux environment does not have `pwsh` installed
  - [x] rerun allocator, text-allocation, and collection benchmarks before and after the change and record same-run C ratios
    ...
    - [x] focused 20-run allocator validation on April 28, 2026: `HeapLocalBucketReuse` ratio 0.829, `SystemMemoryBucketReallocate` 0.896, and `SystemMemoryFallbackReallocate` 0.855 versus same-run C baselines
    - [x] focused 20-run text-allocation validation on April 28, 2026: `OwnedPathAllocation` ratio 0.977 and `OwnedTextAllocation` 0.967 versus same-run C baselines
    - [x] focused 20-run collection validation on April 28, 2026: `DictionaryLookup` ratio 1.049, `ListGrowth` 1.051, `ListIteration` 1.077, `QueueGrowth` 1.024, `StackGrowth` 1.008, and `LinkedListPush` 1.061 versus same-run C baselines
  - Risk: allocator complexity, alignment/header correctness, memory retention, platform-specific behavior.

- [x] Add `LinkedList.ReserveNodes(count)` or allocator-aware construction
  - [x] provide an explicit way for users to request predictable linked-list node allocation when the required node count is known
  - [x] decide whether reservation belongs on `LinkedList<T>` directly, on an allocator-backed constructor, or on a reusable node-pool allocator
    - Initial implementation uses `LinkedList<T>.ReserveNodes(count)` as the narrow API: it pre-fills the list allocator's node bucket without adding per-list pool state.
  - [x] keep the default linked-list API simple while documenting the performance knob for allocation-heavy workloads
  - [x] benchmark reserved linked-list push/pop against C and Rust baselines
    - [x] add `benchmarks/collections/LinkedListReservedPush` with Stark `ReserveNodes(4096)` before build-and-drain and natural C/Rust linked-list baselines; current process-level timing includes reservation cost rather than isolating post-reserve hot-loop cost
    - [x] focused 5-run smoke on April 28, 2026: Stark `LinkedListReservedPush` averaged 1946 us versus C at 1644 us, ratio 1.184; this is expected to be worse end-to-end because reservation cost is included and the API is meant to move allocation out of a later hot path
...
- [x] Promote `TryConvertAsciiToUnicode` literal specialization into a full SSA optimization feature
  ...
  - [x] extend the SSA value-fact model with known text literal payload facts
    ...
    - [x] record decoded UTF-32 payload for proven `unicode` literals when useful for copy lowering
    - [x] record exact literal length and ASCII-only status together so the optimizer can distinguish ASCII-only literals from general UTF-8 literals
    - [x] fold `System.Text.AsciiLength`/`UnicodeLength` calls on known text literals during SSA constant propagation so loop bounds and arithmetic users see integer constants before LLVM lowering
  - [x] propagate known literal payload facts through simple non-dynamic carriers
    - [x] direct `use`/alias values
    - [x] stack locals initialized from a literal and not overwritten before the call
      - [x] LLVM regression coverage proves the literal call-site specialization still triggers through a stack local carrier
  ...
  - [x] add correctness coverage for literal specialization
    ...
    - [x] empty literal call
    - [x] literal stored in a local
    - [x] literal stored in an immutable local or const
    - [x] phi-joined identical literal payload
    - [x] non-ASCII literal fallback
    - [x] too-small destination
    - [x] null destination
...
- [ ] Add alias-aware memory optimization
  ...
  - [ ] add tests for redundant local loads, field loads, readonly calls, unknown raw-pointer barriers, and dead stores
    ...
    - [x] add SSA and pipeline regression coverage proving source-level fixed-array element lane loads are forwarded while dynamic index stores stay conservative
...
- [ ] Add branch shaping and predication
  - [ ] implement jump threading from proven branch facts
    - [x] thread proven edges through side-effect-free branch-only blocks without rewriting phi edges
  - [ ] merge diamonds into `select` when both arms are pure, cheap, and branch misprediction is likely more expensive than executing both arms
    - [x] add late SSA branch shaping that predicatizes simple scalar return diamonds into LLVM `select`
...
- [ ] Add specialization-driven optimization for full generics and constrained generics
  ...
  - [x] add tests comparing optimized generic abstractions to hand-written monomorphic code
...
- [ ] Add optimization benchmark gates against Rust and C
  ...
  - [x] add non-erased benchmark variants for literal-heavy text conversion cases
    ...
    - [x] compare both public API usage and a raw widening-kernel shape when that distinction is needed to isolate compiler codegen from stdlib boundary overhead
...
- [ ] Optimize `System.Experimental.Memory` move helpers.
  - [ ] implement `MoveBytes` as a true overlap-safe in-place move instead of delegating to `CopyBytes`
  - [ ] implement `MoveCodePoints` as a true overlap-safe in-place move instead of delegating to `CopyCodePoints`
  - [ ] use forward copy when destination starts before source and backward copy when destination starts inside/after the source range
  - [ ] preserve zero-count and disjoint fast paths
  - [ ] verify the generated LLVM does not allocate a dynamic snapshot for `MoveBytes` or `MoveCodePoints`
  - [ ] add runtime tests covering forward overlap, backward overlap, exact same source/destination, disjoint move, and zero-count move for bytes and code points

- [ ] Add explicit disjoint append fast paths to `System.Experimental.Memory`.
  - [ ] add `AppendBytesDisjoint` for callers that can prove source and destination storage do not overlap
  - [ ] add `AppendCodePointsDisjoint` for callers that can prove source and destination storage do not overlap
  - [ ] keep the existing overlap-safe `AppendBytes` and `AppendCodePoints` APIs for aliased source slices
  - [ ] ensure disjoint append helpers use `disjoint` parameters and `independent` loops where valid
  - [ ] after reserve, form explicit tail `init` slices such as `init storage[oldLength, count]` instead of appending through `storage.Length` inside the copy loop
  - [ ] route disjoint byte and code point appends through `InitializeBytesDisjoint` and `InitializeCodePointsDisjoint` so optimized builds lower to `llvm.memcpy` where applicable
  - [ ] route append-fill helpers through tail `init` slices so optimized builds lower to `llvm.memset` where applicable
  - [ ] update dynamic-reserve benchmarks to use the disjoint append APIs when the source is stack-owned and independent from the dynamic destination
  - [ ] add LLVM checks proving the disjoint append helpers avoid runtime pointer-range disjoint checks and snapshot allocation
  - [ ] add LLVM checks proving hot append loops do not retain a loop-carried `storage.Length` dependency after optimization

- [ ] Rewrite experimental append-through-length loops into explicit destination regions.
  - [ ] update `OwnedAscii.AppendSlice`, `OwnedAscii.AppendAscii`, `OwnedUnicode.AppendSlice`, and `OwnedUnicode.AppendUnicode` to reserve once, capture the old length, create a tail `init` slice, and copy into that region
  - [ ] update `DynamicByteBuffer.WriteSlice` and `DynamicByteBuffer.WriteFill` to reserve once and write into a tail region rather than repeatedly appending to `self.Storage[self.Storage.Length]`
  - [ ] update `Path.TryJoin` and separator normalization to write into planned destination ranges after one reserve instead of appending through `destination.Bytes.Length`
  - [ ] update dictionary and ring-queue capacity initialization loops to use tail initialization regions where safe
  - [ ] keep append-through-length only for genuinely scalar one-element appends where the extra region setup is not profitable
  - [ ] add LLVM checks for memcpy/memset recognition, removed length-update loop carries, and unchanged ownership/drop behavior

- [ ] Audit experimental loops for `independent` eligibility.
  - [ ] mark disjoint copy and fill true-branches as `independent` when they are not replaced by memory intrinsics
  - [ ] mark dictionary state/value initialization loops independent when each iteration writes a distinct slot
  - [ ] mark ASCII-to-Unicode widening loops independent when source and destination regions are proven separate and the conversion is one-to-one
  - [ ] do not mark variable-length UTF-8 or UTF-16 decode/encode loops independent when write indexes are data-dependent
  - [ ] add LLVM checks for `llvm.loop.parallel_accesses` on retained loops that are meant to vectorize

- [ ] Reduce experimental memory helper call overhead.
  - [ ] mark non-allocating bounded helpers `finite` where the implementation can prove termination
  - [ ] review `inline`/`inlinehint` annotations on hot `System.Experimental.Memory` helpers and make the benchmark-critical wrappers inline aggressively enough
  - [ ] audit imported helper declarations so function effects preserve `willreturn`, `mustprogress`, memory effects, `nocapture`, readonly/writeonly, and noalias facts across module boundaries
  - [ ] add regression tests comparing same-module helper calls and imported stdlib helper calls for equivalent LLVM attributes

- [ ] Reduce `MemoryStatus` overhead in hot infallible helper paths.
  - [ ] identify helpers that always return `MemoryStatus.Ok` after local validation, such as initialized fill and disjoint copy
  - [ ] either expose infallible variants for those operations or ensure status construction plus caller `Ok(...)` checks are folded away at `-O3`
  - [ ] add LLVM checks that hot copy/fill benchmark loops do not retain unnecessary enum switch branches after optimization

- [ ] Improve benchmark confidence for experimental standard library comparisons.
  - [ ] interleave `stark` and `stark-experimental` executions within each benchmark group to reduce order, thermal, and cache effects
  - [ ] report or gate on medians in addition to averages for noisy allocator benchmarks
  - [ ] keep raw allocator benchmarks that are implementation-identical from being interpreted as experimental standard-library regressions unless the generated LLVM differs materially
  - [ ] add a focused benchmark or IR gate proving `MemoryCopyFill` experimental move paths no longer allocate snapshots

- [ ] Optimize experimental fixed runtime buffers.
  - [ ] replace heap-backed `dynamic` storage in `FixedByteBuffer512`, `FixedByteBuffer4096`, and `FixedByteBuffer8192` with true inline fixed storage, or make fixed storage initialization one-time and allocation-free
  - [ ] remove repeated `EnsureFixedStorage` checks from hot read/write paths after construction or reset establishes the buffer invariant
  - [ ] preserve the existing read cursor, write cursor, clear, reserve, and slice APIs
  - [ ] add LLVM checks proving fixed buffer byte reads and writes do not call dynamic reserve/allocation helpers
  - [ ] add benchmarks covering fixed-buffer write, read, clear, and reuse behavior

- [ ] Optimize experimental runtime buffer slice movement.
  - [ ] add disjoint `WriteSlice` fast paths that use `disjoint` parameters and `independent` loops when callers can prove non-overlap
  - [ ] keep the existing overlap-safe write path for aliased source slices
  - [ ] rewrite `WriteSliceToStorage` to copy into `storage[writePosition, count]` so the destination range is explicit to the optimizer
  - [ ] rewrite `WriteFillToStorage` to fill `storage[writePosition, count]` through an explicit destination region
  - [ ] rewrite buffer compaction to use the optimized in-place move helpers instead of snapshot allocation
  - [ ] add tests covering aliased writes, disjoint writes, partial compaction, full compaction, and empty compaction
  - [ ] add LLVM checks proving disjoint slice writes and compaction avoid dynamic snapshots

- [ ] Optimize experimental TCP buffer reads.
  - [ ] avoid the scratch-buffer plus per-byte `WriteByte` loop in `TcpClient.Read(FixedByteBuffer*)`
  - [ ] read directly into the destination writable range when the platform API can safely target it
  - [ ] otherwise perform one reserve and one slice write instead of one status-producing write per byte
  - [ ] expose writable tail slices for fixed and dynamic byte buffers so socket reads can target contiguous spare capacity directly
  - [ ] apply the same reserve-once strategy to dynamic buffer reads
  - [ ] add correctness tests for returned byte counts, cursor advancement, zero-byte reads, and partial reads
  - [ ] add LLVM or benchmark checks proving the hot path has no per-byte `MemoryStatus` branch

- [ ] Optimize experimental console byte writes.
  - [ ] replace byte-by-byte `OwnedAscii` construction in `WriteBytesToStdout` and `WriteBytesToStderr` with a direct byte-slice platform write path where supported
  - [ ] if a direct platform write is not available yet, construct a transient non-owning ASCII view without allocating or copying
  - [ ] coalesce `WriteLine` newline emission with the preceding write when the destination supports a single contiguous byte slice
  - [ ] add tests for stdout, stderr, empty writes, newline writes, and invalid byte handling
  - [ ] add console benchmarks that distinguish formatting cost from platform write cost

- [ ] Optimize experimental linked list allocation paths.
  - [ ] add a fresh-node fast path for append/prepend when reserved capacity is available and no removed node needs to be reused
  - [ ] stop eagerly initializing the free-list for every reserved node; use the free-list only for nodes that have actually been removed
  - [ ] reduce `LinkedListValueSlot<T>` enum/switch overhead when ownership and drop safety can be preserved without a value tag on every access
  - [ ] keep churn behavior correct for removed-node reuse, clear, drop, and iteration
  - [ ] add benchmarks and LLVM checks for linked-list push, reserved push, churn, pop-only, and iteration

- [ ] Restore ring-buffer behavior for experimental queue hot paths.
  - [ ] replace the default experimental `Queue.TryDequeue` `MoveAt(0)` behavior with head/length ring-buffer semantics
  - [ ] keep `RingQueue` as an explicit type if it still adds API value, but make the default FIFO queue efficient
  - [ ] ensure enqueue, dequeue, peek, clear, reserve, wraparound, and growth preserve item ownership correctly
  - [ ] add dequeue-heavy and churn benchmarks in addition to growth-only benchmarks
  - [ ] add tests that cover wraparound growth, full-to-empty transitions, and repeated reuse

- [ ] Reduce experimental dictionary access overhead.
  - [ ] investigate replacing per-value `DictionaryValueSlot<V>` switches with separate occupancy metadata plus initialized value storage
  - [ ] preserve drop safety, moved-out value safety, tombstone reuse, and probe-chain correctness
  - [ ] add lookup APIs such as `TryGetRef` or contains-plus-reference helpers where they avoid copying large values
  - [ ] add const-provenance lookup variants for key graphs that can be deeply readonly without changing the ordinary `borrow` APIs
  - [ ] review `DictionaryKey.Hash` and `DictionaryKey.Equals` calls so readonly/law/const facts remain visible through doctrine dispatch
  - [ ] review remove/update paths for avoidable value moves and status branches
  - [ ] add benchmarks for lookup, insert, update, remove, and mixed workloads

- [ ] Add static disjoint fast paths to experimental text append APIs.
  - [ ] add disjoint/literal append variants for `OwnedAscii` and `OwnedUnicode` callers with statically non-aliased input
  - [ ] route literal, stack-owned, and independent source appends through the disjoint variants
  - [ ] use tail `init` slices after reserve so text append copies expose fixed source and destination ranges
  - [ ] make literal append helpers preserve literal length and const-provenance facts through `AsciiView` and `UnicodeView`
  - [ ] keep existing overlap-safe append APIs for aliased slices
  - [ ] use `disjoint` parameters and `independent` loops where valid
  - [ ] add LLVM checks proving common text appends avoid runtime alias checks and snapshot allocation

- [ ] Add const-provenance fast paths for experimental text and path APIs.
  - [ ] add const/literal-specialized overloads or internal helpers for text append, path join, path normalization, and path queries without replacing the general `borrow` APIs
  - [ ] route string literals, immutable locals, const globals, and known static platform separator strings through const-preserving helpers
  - [ ] ensure const parameters are used only where callers can actually supply permanent const provenance
  - [ ] combine `const` with `disjoint` on immutable source plus mutable destination helpers when both facts are true
  - [ ] add LLVM checks proving const/literal paths retain readonly, nocapture, and constant length/data facts through imported stdlib calls

- [ ] Optimize experimental path construction and normalization.
  - [ ] add disjoint/literal fast paths for `Path.TryJoin` and separator normalization
  - [ ] reserve destination capacity once, then write directly without snapshotting independent inputs
  - [ ] inline separator insertion in the hot path so it does not leave an avoidable `MemoryStatus` switch
  - [ ] prefer `PathFacts` or combined query helpers when callers need extension, base name, and directory name from the same path
  - [ ] add benchmarks for join, normalize, separate path queries, and combined path queries

- [ ] Optimize experimental Unicode formatting.
  - [ ] implement direct Unicode digit emission for `TryFormatI1024Unicode`, `TryFormatU1024Unicode`, and the I128/U128 Unicode wrappers
  - [ ] remove the temporary ASCII formatting plus ASCII-to-Unicode conversion pass on hot integer formatting paths
  - [ ] mark formatting wrappers `finite` where the digit loop bounds prove termination
  - [ ] write formatted digits into planned destination ranges rather than appending through `OwnedUnicode.CodePoints.Length`
  - [ ] add tests for sign handling, zero, min/max values, and large powers of ten
  - [ ] add benchmarks and LLVM checks proving direct Unicode formatting avoids the temporary ASCII buffer

- [ ] Use bounded raw pointer region facts at experimental encoding and platform edges.
  - [ ] replace internal unbounded raw pointer helpers such as UTF-8/UTF-16 encode/decode helpers with bounded `rawptr<T>[count]` and `rawmutptr<T>[capacity]` parameters
  - [ ] add `where disjoint(...)` contracts for source and destination raw regions when conversion helpers can require non-overlap
  - [ ] keep raw pointers contained at FFI/platform boundaries and convert to slices or bounded raw regions immediately inside the stdlib
  - [ ] update file, console, TCP, and filesystem direct byte paths to preserve slice length, mutability, readonly, and noalias facts up to the platform call
  - [ ] add diagnostics or tests preventing unbounded raw pointer helpers from creeping back into non-FFI stdlib internals

- [ ] Improve cross-module optimization visibility for the experimental standard library.
  - [ ] preserve imported function summaries for `willreturn`, `mustprogress`, memory effects, `nocapture`, readonly/writeonly, and noalias facts across module boundaries
  - [ ] review experimental memory, runtime, text, IO, console, net, and collection helper declarations for missing effect and alias facts
  - [ ] consider link-before-opt, LTO-style stdlib body visibility, or summary-based inlining for benchmark-critical stdlib helpers
  - [ ] add regression tests comparing same-module helper calls and imported experimental stdlib calls for equivalent optimizer attributes

- [ ] Expand experimental standard library benchmark gates.
  - [ ] add focused gates for fixed runtime buffers, runtime buffer compaction, TCP reads, console byte writes, linked-list push/churn, queue dequeue/churn, dictionary mixed workloads, text append, path join/normalize/query, and Unicode formatting
  - [ ] make each gate compare `stark` and `stark-experimental` with the same benchmark name and only the language column differing
  - [ ] include generated LLVM or call-count checks for the optimizations where wall-clock noise hides the root cause
